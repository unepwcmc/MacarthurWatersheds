...
class Backbone.Views.MapView extends Backbone.View
  ...
  initQueryLayer: (geo) ->
    @collection = topojson.feature(geo, geo.objects.macarthur_watershed)
    @interiors = topojson.mesh(geo, geo.objects.macarthur_watershed, (a, b) -> a != b )
    @queryLayer = L.geoJson(@collection, {style: @baseStyle}).addTo(@map)
    L.geoJson(@interiors, {style: @baseStyle}).addTo(@map)
